...
async function processFinalEmotion(responses: Array<{ question: string; answer: string }>) {
  // Create a detailed context with both questions and answers
  const contextPairs = responses.map(r => `Question: "${r.question}" - Answer: "${r.answer}"`);
  const contextString = contextPairs.join("\n");
  
  const prompt = `Based on this conversation:\n${contextString}\n\nDetermine the user's dominant emotion based on these question-answer pairs. Choose EXACTLY ONE emotion from this list of 41 emotions: ${validEmotions.join(", ")}. Respond with only the emotion name, no explanation or additional text.`;
