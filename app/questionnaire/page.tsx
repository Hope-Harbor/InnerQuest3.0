    ...
    async function initialize() {
      ...
      try {
        ...
        const initialPrompt = currentLang === 'en'
          ? `As a balanced and neutral psychologist, create a thoughtful and creative question for a ${storedRole} about their mental well-being at this moment/today that can be answered with Yes, No, or Maybe.It can be about any aspect of life. Limit to 20 words, no quotes.`
          : `身為一位心理諮商師，請用繁體中文為一位${
              storedRole === 'teenager' ? '青少年' : 
              storedRole === 'undergraduate' ? '大學生' : 
              storedRole === 'workingProfessional' ? '在職人士' : '在職人士'
            }提出一個關於當下/今日心理健康的問題，該問題可以用是、否或也許來回答。限制在20字以內，不要使用引號。`;
  ...
  const handleAnswer = async (answer: string) => {
    try {
      ...
      const nextPrompt = language === 'en'
        ...
        userRole === 'working' ? '在職人士' : '在職人士'
          }提出一個關於當下/今日心理健康的問題，該問題可以用是、否或也許來回答。限制在20字以內，不要使用引號。可以是來自任何方面關於生活的`;
          ...
          <h2 className="welcome-heading">
            {language === 'zh-TW' ? 
              `歡迎，${userRole === 'teenager' ? '青少年' : 
                       userRole === 'undergraduate' ? '大學生' : userRole === 'working' ? '在職人士' : '在職人士'}!` :
              `Welcome, ${
                userRole === 'teenager' ? 'teenager' : 
                userRole === 'undergraduate' ? 'undergraduate student' : 
                userRole === 'working' ? 'working professional' : 
                userRole
              }!`
            }
          </h2>
